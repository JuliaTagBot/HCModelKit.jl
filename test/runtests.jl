...
@testset "HCModelKit.jl" begin
    @testset "System (no parameters)" begin
        ...
        @test F(v) ≈ û
        @test interpreted(F)(v) ≈ û
        @test evaluate(interpreted(F), v) ≈ û
        @test evaluate!(u, F, v) ≈ û
        @test u ≈ û
        Û = FD.jacobian(fdm, F_test, v)[1]
        @test jacobian(F, v) ≈ Û
        @test jacobian(interpreted(F), v) ≈ Û
    ...
    @testset "System (parameters)" begin
        ...
        @test F(v, p) ≈ û
        @test evaluate(interpreted(F), v, p) ≈ û
        @test interpreted(F)(v, p) ≈ û
        @test evaluate!(u, F, v, p) ≈ û
        @test u ≈ û
        Û = FD.jacobian(fdm, v -> F_test(v, p), v)[1]
        @test jacobian(F, v, p) ≈ Û
        @test jacobian(interpreted(F), v, p) ≈ Û
        @test jacobian!(U, F, v, p) ≈ Û
        @test U ≈ Û rtol = 1e-8
        u .= 0
        U .= 0
        @test evaluate_jacobian!(u, U, F, v, p) === nothing
        @test u ≈ û
        @test U ≈ Û
        v, V = evaluate_jacobian(F, v, p)
        @test v ≈ û
        @test V ≈ Û
    end
    ...
    @testset "Homotopy" begin
        ...
        @test H(v, s) ≈ û
        @test evaluate(interpreted(H), v, s) ≈ û
        @test interpreted(H)(v, s) ≈ û
        @test evaluate!(u, H, v, s) ≈ û
        @test u ≈ û
        Û = FD.jacobian(fdm, v -> H_test(v, s), v)[1]
        @test jacobian(H, v, s) ≈ Û
        @test jacobian(interpreted(H), v, s) ≈ Û
    ...
    @testset "Homotopy (parameters)" begin
        ...
        @test H(v, s, p) ≈ û
        @test evaluate(interpreted(H), v, s, p) ≈ û
        @test interpreted(H)(v, s, p) ≈ û
        @test evaluate!(u, H, v, s, p) ≈ û
        @test u ≈ û
        Û = FD.jacobian(fdm, v -> H_test(v, s, p), v)[1]
        @test jacobian(H, v, s, p) ≈ Û
        @test jacobian(interpreted(H), v, s, p) ≈ Û
        @test jacobian!(U, H, v, s, p) ≈ Û
        @test U ≈ Û rtol = 1e-8
        u .= 0
        U .= 0
        @test evaluate_jacobian!(u, U, H, v, s, p) === nothing
        @test u ≈ û
        @test U ≈ Û

        û = FD.grad(fdm, s -> H_test(v, s, p), s)[1]
        @test dt(H, v, s, p) ≈ û
        @test dt(interpreted(H), v, s, p) ≈ û
        @test dt!(u, H, v, s, p) ≈ û
        u .= 0
        U .= 0
        @test dt_jacobian!(u, U, H, v, s, p) === nothing
        @test u ≈ û
        @test U ≈ Û

        v, V = dt_jacobian(H, v, s, p)
        @test v ≈ û
        @test V ≈ Û
    end
